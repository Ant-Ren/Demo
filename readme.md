...
### Business Logic
- The key logic is to get the top 20 stakes per bet offer, and a customer can post additional stakes for the same betting offer. Also, a customer ID can appear at most once in the top 20 stakes. This alerts me to pay attention to these points:
...
- Requrement mentioned the service need to be able to handle a lot of simultaneous requests, so business logic implementation should also consider the performance seriously.
  ...
  - Ultimately, I chose the pre-sorting strategy: when each customer post a bet on an offer, stakes are sorted in descending order ahead of time. This sacrifices some performance during stake intake but provides much better performance for real-time fetching and updating of the top N stakes. From a casino business risk management perspective, I believe this trade-off is more beneficial than harmful.
